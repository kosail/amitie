...
## [2026-09-13] feature — amount-banded plazos and a confirmed user-stated term
- agent: opencode / deepseek-flash
- requirements: REQ-LM-10, REQ-LM-11 (behavior; no contract change)
- invariants: none
- files: `backend/engine/loan_offer.py`, `backend/agent/loans.py`, `backend/agent/loans_fallback.py`, `backend/api/routers/loans.py`, `backend/mcp_servers/finance/{service,server}.py`, `backend/tests/test_loan_offer.py`, `backend/tests/test_loans_consult.py`, `A2UI_CATALOG.md`, `backend/LOANS_CONSULT_GUIDE.md`
- decision: Two problems. (1) The plazo cards were always the same 6/12/24/36/48 regardless of amount, so a small loan showed nonsensical long terms. Added `allowed_terms_for(amount)` amount bands (<15k → 6/12; 15k–50k → 6/12/24; 50k–100k → 12/24/36; ≥100k → 12/24/36/48); `term_options` now builds from the band, always adding the selected and any user-requested term. (2) A user-stated term was ignored (`compute_loan_offer` never received it and `create_loan` overwrote the passed term with the stored offer's). Added `_extract_term` (N meses / N años / un año / año y medio), conversation state (`loan_requests.context_json`: `requested_term`, `term_confirmed`) loaded/saved by the router and passed to the service, and a deterministic confirmation turn: while a term is requested but unconfirmed the service returns `terminal_response = null` with a message that acknowledges the term and states the real implications from the engine (monthly payment, its share of income, total interest, comparison to the recommendation; if it exceeds capacity, says so and offers alternatives). Only on confirmation does the terminal build at that term. `propose_offer(requested_term=...)` presents the offer at the selected term and returns `recommendation.months` (selected), `engineRecommended`, `requested`, and a `capacity` block; `option.requested` rides the scenario. `create_loan` now honors the chosen term (validated 6–48) instead of forcing the stored offer's. The prompt now states the conversation state, instructs the model to respect a user-stated term and stay consistent with prior messages, and to use the amount-banded options.
- rationale: The offered plazos must reflect the amount and the user's explicit request, and the assistant must react to the user (confirm + explain) rather than silently substitute its own term — all grounded in the profile/context already computed by the engine. The confirmation is enforced deterministically (not left to the model) so it can't be skipped.
- impact: New engine tests (amount bands; requested term kept/selected; propose_offer options) and consult tests (confirmation turn then honored term; `create_loan` honors the requested term). Full suite 238 passing (was 234). `A2UI_CATALOG.md` §6 and `LOANS_CONSULT_GUIDE.md` §5.2/§6.1 updated. Frontend change tracked in `openspec/changes/show-loan-term-cards`.
- follow_ups: Consider surfacing the minimum/maximum when a user asks for an out-of-range term; optionally let tapping a card select a term (currently conversational only).

## [2026-09-13] fix — loans intake: never jump to the maximum; honor the requested amount
- agent: opencode / deepseek-flash
- requirements: REQ-LM-10, REQ-LM-11 (behavior; no contract change)
- invariants: none
- files: `backend/agent/loans.py`, `backend/agent/loans_fallback.py`, `backend/api/routers/loans.py`, `backend/config.py`, `backend/.env.example`, `backend/tests/test_loans_consult.py`, `backend/LOANS_CONSULT_GUIDE.md`
- decision: The consult offered the maximum whenever the AI was unsure. Two causes: (1) the requested amount was never persisted, so `consult` extracted it only from the current message and any later turn called `compute_loan_offer(requested_amount=0)` → `amount = max_principal` (the DB showed persisted `u_don` terminals at exactly the max); (2) `loans_fallback.build_terminal` always fabricated an affordable offer from that max when the model failed/timed out/low-confidence, and the prompt marked the offer "obligatoria". Fix: persist a negotiation state (`requested_amount`, `purpose`, `use_max`, `requested_term`, `term_confirmed`, `purpose_asked`) in `loan_requests.context_json` (router loads/saves) and use `effective_amount = _extract_amount(text) or state.requested_amount`. Added a deterministic intake gate: without an amount (and no explicit maximum) the service asks naturally for the amount/purpose and returns `terminal_response = null` without building or persisting an offer; the model writes the question (`_intake_response` + an intake system prompt) with `loans_fallback.intake_response` as the deterministic fallback, and `build_terminal` now asks instead of offering when there is no amount. Added `_wants_max` ("el máximo", "lo que me puedas dar", "tú dime") so an explicit maximum is still honored. Prompt: conditional offer mandate (only once the amount is known), intake instruction, state included. Term confirmation now bundles the purpose question to avoid an extra turn.
- rationale: The offered amount must reflect what the user asked for; when the assistant does not understand it should keep the conversation going (ask) instead of silently offering the ceiling. Gating deterministically (not just via the prompt) makes the guarantee independent of model behavior, while the model still writes the conversational question.
- impact: New tests: vague turn asks (no terminal); amount persists and never jumps to max (5,000 stays 5,000 through a term confirmation); explicit maximum offered; fallback intake; latency defaults. Full suite 243 passing (was 238). No invariant/API-shape change — an intake turn reuses `{response_text, terminal_response: null}`.
- follow_ups: Frontend client timeout raised to 12 s (`openspec/changes/loans-intake-flow`); consider streaming or a smaller model call if latency still hurts.

## [2026-09-13] change — loans latency budget (the 4s deadline triggers)
- agent: opencode / deepseek-flash
- requirements: REQ-LM-12 (latency behavior)
- invariants: none
- files: `backend/config.py`, `backend/.env.example`, `backend/agent/loans.py`, `backend/api/app.py`, `backend/tests/test_loans_consult.py`
- decision: Under `PR_SWITCH=1` the 4.0 s deadline with `LOANS_MAX_TOKENS=900` and a large prompt (full analysis, the offer including the entire per-month schedule, full history) made the DeepSeek call time out or truncate the JSON, so `consult` frequently fell back to the canned engine offer — the main source of the "stiff" feel. Raised `LOANS_LLM_DEADLINE_SECONDS` to 6.5 and `LOANS_MAX_TOKENS` to 1500, added `LOANS_INTAKE_DEADLINE_SECONDS=3.5` for the short intake turn, trimmed the per-month `schedule` (the model binds `/loan/schedule`) and the analysis payload from the prompt, capped history to the last 8 turns, and wired the trace/log to record the real fallback reason (timeout/provider_error/invalid_output/low_confidence) instead of `error=None`.
- rationale: Keep the model in the conversation loop without losing the deterministic safety net; the 6.5 s budget plus local TTS stays under the frontend's 12 s abort.
- impact: `api/app.py` passes the intake deadline; tests assert the new defaults and that the prompt no longer carries the long schedule. Frontend client timeout raised to 12 s.
- follow_ups: Tune the numbers with real provider latency; keep them env-only.

## [2026-09-13] fix — agent turn no longer fails hard at the LLM call limit
- agent: opencode / deepseek-flash
- requirements: REQ-NFR-01 (resilience); REQ-API-02 (error contract)
- invariants: none
- files: `backend/config.py`, `backend/.env.example`, `backend/agent/service.py`, `backend/agent/model.py`, `backend/tests/test_agent_service.py`
- decision: `POST /api/message` failed with ADK's `LlmCallsLimitExceededError: Max number of llm calls limit of 6 exceeded`. The general La Mesa agent is an ADK function-calling loop (one LLM round trip per MCP tool: `get_financial_context` → `analyze_loans` → `get_audience` → `simulate_plan` → `detect_plan_breaks` → `persist_ui`), and the running `.env` pinned `AGENT_MAX_MODEL_CALLS=6`, so the loop hit the ceiling before `persist_ui` and the ADK error fell into the generic `except Exception` → `error_code:"agent_error"`. Fixes: (1) raised the per-turn budget to 12 (`.env.example` and the `Settings` default; the running `.env` must also be updated and the process restarted); (2) `AgentService.run_turn` now detects ADK's limit error by walking the exception chain (`LlmCallsLimitExceededError`/`ModelCallLimitError` by class name, plus the `DynamicNodeFailError.error` link) and returns `{status:"error", error_code:"model_call_limit", retryable:true}` instead of a generic error; (3) `GatewayLlm` exposes `calls`/`limit` and logs one `agent_model_call` per iteration (index, max, tool names), and the service logs `agent_model_call_limit` when the budget is exhausted.
- rationale: The multi-call behavior is inherent to the agentic ADK/MCP design (INV-011/014/015), not a bug; the bug was a budget pinned below the flow's needs plus a mis-typed error. Keeping the tool sequence unchanged (per owner) and raising the budget resolves the failure while leaving MCP as the only door.
- impact: A limit-overrun turn now degrades to a clean, retryable `model_call_limit` error (the frontend shows its handled error) rather than an empty/agent_error turn, and the ADK traceback is accompanied by a clear `agent_model_call_limit` warning. New tests: an always-tool-calling provider with `max_model_calls=3` reproduces the ADK limit and asserts `model_call_limit`/`retryable`, and a config-default assertion (>=12). Full suite 245 passing (was 243).
- follow_ups: **Operational:** set `AGENT_MAX_MODEL_CALLS=12` in the running `.env` (the trace ran from `/external/storage/Workspaces/Playground/amitie/backend`, a different checkout than the one edited here — apply the change there too) and restart. Optional later: batch the reads / drop the redundant `detect_plan_breaks` to cut the loop from ~6 calls to ~4.
