...
```
            ...
            │      ├── hydration/     placeholder hydration + revalidation│
...
# 4. Run the test suite (143 tests expected to pass)
...
```sh
# from the backend/ directory (you are already there if the prompt shows ••/amitie/backend)
# first time: python -m db.init
# optional: cp .env.example .env  and set GEMINI_API_KEY (needed for agent turns, not for /docs)
...
- Export to file: `python -m api.openapi --out openapi.json`

Generate a typed client from that spec (Orval, openapi-generator, etc.). The
frozen product routes are `SPECS.md` §8, not every path in the schema.
